...
namespace AtividadePratica
{
    public class Grafo
    {
        ...
        public int getGrau(int v1)
        {
			...
			foreach (var item in NOMESVERTICENaoDirigido)
			{
				if (item.Split("-")[0].ToString() == v1.ToString() || item.Split("-")[1] == v1.ToString())
					cont++;
			}        
			return cont++;
        }		
        ...
        {
			if (Loop() || isParalelo() || !isRegular())
				return false;
			int qtdGrau = getGrau(TodosVertice[0]);//1 vertice
			
			return TodosVertice.Count - 1 == qtdGrau ;
		}
		...
		public bool AlgoritmoPrim(int vertice) 
		{
			//Raiz da Ã¡rvore
			if (!isConexo())
			{
				Console.WriteLine("O algoritmo de Prim é para grafos conexos");
				return false;
			}
            ...
            foreach (string item in NOMESVERTICENaoDirigido)
            {
		...
		public bool AlgoritmoKurskal(int vertice)
		{
			if (!isConexo())
			{
				Console.WriteLine("O algoritmo de kurskal é para grafos conexos");
				return false;
			}
